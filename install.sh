...
getAppVersion(){
    version=$(sudo curl -Ls "https://api.github.com/repos/alie110/rocket-ssh2/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
    echo $version;
}
...
copyPanelRepo(){
   ...
   link=https://raw.githubusercontent.com/alie110/rocket-ssh2/main/app.zip
...
installationInfo(){
    clear
    echo -e "\n"
    bannerText=$(curl -s https://raw.githubusercontent.com/alie110/rocket-ssh2/master/rocket-banner.txt)
    printf "%s" "$bannerText"
    echo -e "\n"
